#!/usr/bin/env python3
"""Smoke test for the skill-usage renderer pipeline.
...
Steps:
  1. Validate tests/fixtures/usage_data.sample.json against the schema.
  ...
  4. Assert report.html contains the chart section headings + inline <svg>.
...
SCHEMA = ROOT / "analysis" / "usage_data.schema.json"
FIXTURE = HERE / "fixtures" / "usage_data.sample.json"
...
def run_render() -> list:
    ...
    failures = []
    with tempfile.TemporaryDirectory(prefix="skill-usage-smoke-") as tmp:
        tmp_path = Path(tmp)
...
def main() -> int:
    print("=" * 64)
    print("skill-usage renderer smoke test")
    print("=" * 64)
    print()
    ...
    if f2:
        ...
        print("    PASS (2 artifacts produced, chart headings present)")
    print()
    ...
    print("usage_data.json. Safe to proceed.")
    return 0
